...
/**
 * Draws borders around the given d message along with additional information such as :
 *
 * <ul>
 ...
 *       .logStrategy(customLog) // (Optional) Changes the d strategy to print out. Default LogCat
 *       .tag("My custom tag")   // (Optional) Global tag for every d. Default PRETTY_LOGGER
 *       .build();
 * </code></pre>
 */
public class PrettyFormatStrategy implements FormatStrategy {
  ...
  /**
   * Android's max limit for a d entry is ~4076 bytes,
  ...
  private int getStackOffset(@NonNull StackTraceElement[] trace) {
    ...
    for (int i = MIN_STACK_OFFSET; i < trace.length; i++) {
      ...
      if (!name.equals(LoggerPrinter.class.getName()) && !name.equals(LogAndroid.class.getName())) {
        return --i;
      }
    }
    return -1;
  ...
  public static class Builder {
    ...
    @Nullable String tag = "LogAndroid";
